...
def detect_contours(img_pil):
    ...
    for c in cnts:
        ...
        dup = any(
            ...
            max((x2-x1)*(y2-y1)+(bx2-bx1)*(by2-by1)-
                max(0,min(x2,bx2)-max(x1,bx1))*max(0,min(y2,by2)-max(y1,by1)),1) > 0.7
...
def run_ocr(img_pil, reader):
    return [(t.strip(), float(c))
            for (_,t,c) in reader.readtext(np.array(img_pil)) if c > 0.35]
...
def parse_receipt(lines):
    ...
    skip = {'total','subtotal','sub total','sub-total','tax','change',
            'cash','card','tip','visa','mastercard','amex','balance','due'}
    for line in texts:
    ...
    return {"store_name":store,"date":date or "Unknown","items":items,
            "subtotal":subtotal,"tax":tax,"total":total,"raw_lines":texts}
...
def build_excel(receipts):
    ...
    def hc(ws,r,c,v,w=None):
        ...
        cell.font=Font(bold=True,color=HDR,name="Courier New",size=9)
        cell.fill=PatternFill("solid",fgColor=ACC)
    ...
    def dc(ws,r,c,v,bold=False,color=TXT,fmt=None,align="left",bg=None):
        cell=ws.cell(row=r,column=c,value=v)
        cell.font=Font(bold=bold,color=color,name="Courier New",size=9)
    ...
    ws["A1"].value="RECEIPTIQ  ·  SUMMARY REPORT"
    ws["A1"].font=Font(bold=True,color=HDR,name="Courier New",size=13)
    ws["A1"].fill=PatternFill("solid",fgColor=ACC)
    ws["A1"].alignment=Alignment(horizontal="left",vertical="center")
    ws.merge_cells("A2:G2")
    ws["A2"].value=f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d  %H:%M:%S')}"
    ws["A2"].font=Font(color=DIM,name="Courier New",size=8)
    ws["A2"].fill=PatternFill("solid",fgColor=BG)
    for i,(h,w) in enumerate(zip(["#","Store","Date","Items","Subtotal","Tax","Total"],
                                  [4,32,18,8,14,14,14]),1): hc(ws,4,i,h,w)
    grand=0.0
    for i,rd in enumerate(receipts):
        r=5+i; bg=ALT if i%2 else BG
        dc(ws,r,1,i+1,align="center",bg=bg,color=DIM)
        dc(ws,r,2,rd["store_name"],bg=bg)
        dc(ws,r,3,rd["date"],bg=bg,color=DIM)
        dc(ws,r,4,len(rd["items"]),align="center",bg=bg,color=DIM)
        dc(ws,r,5,rd["subtotal"] or "",fmt='$#,##0.00',align="right",bg=bg,color=DIM)
        dc(ws,r,6,rd["tax"] or "",fmt='$#,##0.00',align="right",bg=bg,color=DIM)
        tot=rd["total"] or 0
        dc(ws,r,7,tot,fmt='$#,##0.00',align="right",bg=bg,bold=True,color=TOT)
        if isinstance(tot,(int,float)): grand+=tot
    tr=5+len(receipts); ws.row_dimensions[tr].height=20
    ...
    ws2["A1"].font=Font(bold=True,color=HDR,name="Courier New",size=11)
    ws2["A1"].fill=PatternFill("solid",fgColor=ACC)
    ...
    for i,rd in enumerate(receipts):
        ...
        for item in rd["items"]:
            dc(ws2,row,1,i+1,align="center",bg=bg,color=DIM)
            dc(ws2,row,2,rd["store_name"],bg=bg); dc(ws2,row,3,rd["date"],bg=bg,color=DIM)
            dc(ws2,row,4,item["name"],bg=bg)
            dc(ws2,row,5,item["price"],fmt='$#,##0.00',align="right",bold=True,color=TOT,bg=bg)
            row+=1
    ...
    ws3["A1"].font=Font(bold=True,color=HDR,name="Courier New",size=11)
    ws3["A1"].fill=PatternFill("solid",fgColor=ACC)
    for i,(h,w) in enumerate(zip(["Item","Store","Date","Price"],[44,30,16,14]),1): hc(ws3,3,i,h,w)
    item_map={}
    for rd in receipts:
        for it in rd["items"]:
            item_map.setdefault(it["name"].lower().strip(),[]).append(
                {"store":rd["store_name"],"date":rd["date"],"price":it["price"]})
...
def process_image(source, reader, engine, engine_status):
    ...
    for i,crop in enumerate(crops):
        ...
        parsed["crop"]=crop; parsed_list.append(parsed)
...
<div class="hero" role="banner">
  ...
  <p class="hero-desc">Multi-receipt detection &nbsp;·&nbsp; YOLO26 trained model &nbsp;·&nbsp; EasyOCR extraction &nbsp;·&nbsp; Price intelligence &nbsp;·&nbsp; Excel export</p>
</div>""", unsafe_allow_html=True)
...
with col_yolo:
    ...
    if engine is not None:
        ...
        with st.expander("📋 How to deploy your trained YOLO26 weights",expanded=False):
            st.markdown("""
**From your Antigravity training run → Streamlit Cloud:**

1. Locate `best.pt` from your training run (e.g. `runs/detect/train3/weights/best.pt`)
2. Rename it → `receipt_detector.pt`
3. Place it in the **repo root** (same folder as `streamlit_app.py`)
4. Commit & push — the green badge will confirm YOLO26 is active

The app runs fully on contour detection until then.
            """)

# ── Input tabs ────────────────────────────────────────────────
tab_cam,tab_upload=st.tabs(["📷   Camera","📁   Upload"])
with tab_cam:
    st.markdown('<div class="tip"><strong>Tips:</strong> flat dark surface · even lighting, no glare · steady hold · one or multiple receipts per frame</div>',unsafe_allow_html=True)
...
if all_r:
    ...
    for rd in all_r:
        for it in rd["items"]:
            item_map.setdefault(it["name"].lower().strip(),[]).append({"store":rd["store_name"],"price":it["price"]})
    dupes={k:v for k,v in item_map.items() if len(v)>1}
    if dupes:
